...
file_path <- "U:/Toboggan/2025/Report/toboggan_smolt_dataentry_2025.csv"
...
df_fish <- df_fish %>%
  mutate(
    ...
    species = case_when(
      # Coho (wild)
        species == "co-w" ~ "Coho (wild)",
        species == "co-a" & r_caudalclip %in% c("top", "bottom") ~ "Coho (wild)",
      
      # Coho (hatchery)
      species == "co-a" & r_caudalclip == "none" ~ "Coho (hatchery)",
      
      # Other species
      species == "st/rb" ~ "Steelhead or Rainbow trout",
      species == "bt/dv" ~ "Bull trout or Dolly varden",
      species == "ct" ~ "Cutthroat",
...
# # Average and standard deviations of wild vs hatchery coho
# p1 <- ggplot(daily_counts_wild, aes(x = date, y = daily_count, color = site, linetype = site)) +
#   geom_line(size = 1) +
#   geom_point(size = 1.2) +
#   scale_x_date(
#     breaks = seq(min(daily_counts_wild$date, na.rm = TRUE), max(daily_counts_wild$date, na.rm = TRUE), by = "8 days"),
#     date_labels = "%d-%b"
#   ) +
#   labs(
#     title = "Wild",
...
# Daily count of wild smolts at barn and fence sites -----------------------

daily_counts_wild <- df_fish %>%
...
if (nrow(daily_counts_wild) == 0) stop("Error: No data available for CO-W at Barn and Fence sites.")

# Plot daily counts for coho (wild)
ggplot(daily_counts_wild, aes(x = date, y = daily_count, color = site, linetype = site)) +
  geom_line(size = 1) +
  geom_point(size = 1.2) +
  scale_x_date(
    breaks = seq(min(daily_counts_wild$date, na.rm = TRUE), max(daily_counts_wild$date, na.rm = TRUE), by = "8 days"),
...
# # Max and min counts days
# 
# daily_totals_coho_wild <- df_fish %>%
#   filter(species == "Coho (wild)") %>%
#   group_by(date) %>%
#   summarise(
#     total_coho = sum(count, na.rm = TRUE),
#     .groups = "drop"
#   )
# 
# # Day with MOST Coho (wild)
# max_day <- daily_totals_coho_wild %>%
#   slice_max(total_coho, n = 1)
# 
# # Day with LEAST Coho (wild)
# min_day <- daily_totals_coho_wild %>%
#   slice_min(total_coho, n = 1)
# 
# max_day
# min_day
...
daily_counts_hatchery <- df_fish %>%
  filter(species == "Coho (hatchery)", site %in% c("barn", "fence")) %>%
  group_by(date, site) %>%
  summarise(daily_count = sum(count, na.rm = TRUE), .groups = "drop") %>%
  arrange(date)


if (nrow(daily_counts_hatchery) == 0) stop("Error: No data available for CO-W at Barn and Fence sites.")

# Plot daily counts for hatchery
ggplot(daily_counts_hatchery, aes(x = date, y = daily_count, color = site, linetype = site)) +
  geom_line(size = 1) +
  geom_point(size = 1.2) +
  scale_x_date(
    breaks = seq(min(daily_counts_hatchery$date, na.rm = TRUE), max(daily_counts_hatchery$date, na.rm = TRUE), by = "8 days"),
    date_labels = "%d-%b"
  ) +
  labs(
    # title = "Hatchery",
...
# # Max and min counts days
# 
#  daily_totals_coho_hatchery <- df_fish %>%
...
#    )
# 
# # Day with MOST Coho (hatchery)
#  max_day <- daily_totals_coho_hatchery %>%
#    slice_max(total_coho, n = 1)
# 
# # Day with LEAST Coho (hatchery)
#  min_day <- daily_totals_coho_hatchery %>%
#    slice_min(total_coho, n = 1)
# 
#  max_day
#  min_day
...
daily_counts_comb <- df_fish %>%
  filter(species %in% c("Coho (hatchery)", "Coho (wild)"),
         site %in% c("barn", "fence")) %>%   # ← ADD THIS PIPE
  group_by(date, site) %>%
  summarise(daily_count = sum(count, na.rm = TRUE), .groups = "drop") %>%
  arrange(date)


if (nrow(daily_counts_comb) == 0) stop("Error: No data available for CO-W at Barn and Fence sites.")

# Plot daily counts for wild and hatchery combined
ggplot(daily_counts_comb, aes(x = date, y = daily_count, color = site, linetype = site)) +
  geom_line(size = 1) +
  geom_point(size = 1.2) +
  scale_x_date(
    breaks = seq(min(daily_counts_comb$date, na.rm = TRUE), max(daily_counts_comb$date, na.rm = TRUE), by = "8 days"),
    date_labels = "%d-%b"
  ) +
  labs(
    # title = "Combined",
...
# # Max and min counts days
# 
# daily_totals_coho_combined <- df_fish %>%
#   filter(species %in% c("Coho (hatchery)", "Coho (wild)")) %>%
#   group_by(date) %>%
#   summarise(
#     total_coho = sum(count, na.rm = TRUE),
...
#   )
# 
# # Day with MOST Coho (wild + hatchery)
# max_day <- daily_totals_coho_combined %>%
#   slice_max(total_coho, n = 1)
# 
# # Day with LEAST Coho (wild + hatchery)
# min_day <- daily_totals_coho_combined %>%
#   slice_min(total_coho, n = 1)
# 
# max_day
# min_day
...
mortality_summary <- df_fish %>%
  ...
  filter(`Total Accidental` > 0 | `Total Sacrifice` > 0)%>%
  rename(Species = species) %>%   # <-- add this
  arrange(match(Species,          # <-- ADD THIS BLOCK
                c("Coho (wild)",
                  "Coho (hatchery)",
                  "Cutthroat",
                  "Steelhead or Rainbow trout")))  
